<nav class="bg-white z-40 border-gray-100 fixed top-0 w-full">
    <div class="max-w-screen-xl flex flex-wrap items-center justify-between mx-auto p-4">
        <a href="{{ route('feeds') }}" class="flex items-center">
            {{-- <img src="https://flowbite.com/docs/images/logo.svg" class="h-8 mr-3" alt="Flowbite Logo" /> --}}
            <span class="self-center md:text-2xl text-xl font-semibold whitespace-nowrap">SocialNetwork</span>
        </a>
        <div class="flex items-center md:order-2">
            <div class="flex gap-x-4 items-center">
                <div class="relative hidden md:block border rounded-md p-1">
            ...
            <ul class="py-2" aria-labelledby="user-menu-button">
                <li>
                    <button data-modal-target="profileModal" data-modal-show="profileModal" class="block text-left w-full px-4 py-2 text-sm text-gray-700 hover:bg-gray-600 hover:text-slate-100"  type="button">Profile Picture</button>
                    <x-tolink link="{{ route('editView') }}" title="Your Details" class="block px-4 py-2 text-sm text-gray-700 hover:bg-gray-600 hover:text-slate-100" />
                    <x-tolink link="{{ route('logout') }}" title="Logout" class="block px-4 py-2 text-sm text-gray-700 hover:bg-gray-600 hover:text-slate-100" />
                </li>
            </ul>
            </div>
        </div>
        <div class="relative block md:hidden w-full border rounded-md p-1 mt-3">
            <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1" stroke="currentColor" class="w-6 h-6 absolute">
                <path stroke-linecap="round" stroke-linejoin="round" d="M21 21l-5.197-5.197m0 0A7.5 7.5 0 105.196 5.196a7.5 7.5 0 0010.607 10.607z" />
            </svg>
            <input id="searchbar2" placeholder="Search" type="text" class="focus:outline-0 placeholder:italic placeholder:text-xs pl-7 text-sm">
        </div>
    </div>
  </nav>
